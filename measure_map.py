...
from pytorch_frcnn import data_generators
...
def get_map(pred, gt, f):
    ...
    for gt_box in gt:
        if not gt_box['bbox_matched']:  # and not gt_box['difficult']:  # todo: the gstdb dataset doesn't have difficult
...
# path to txt file containing groundtruth
...
# input_shape_features = (None, None, 512)  # 1024 for resnet50
...
#rpn_layers = frcnn.rpn(shared_layers, num_anchors)

#classifier = frcnn.classifier(feature_map_input, roi_input, C.num_rois, nb_classes=len(class_mapping), trainable=True)
...
for idx, img_data in enumerate(test_imgs):
    ...
    for key in bboxes:
        ...
        new_boxes, new_probs = roi_helpers.non_max_suppression_fast(bbox, np.array(probs[key]),
                                                                    max_boxes=64, overlap_thresh=0.3)
